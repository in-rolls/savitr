# Surya-on-M4 speedup — historical findings

This document records the original 2026 investigation. Its timings describe
the exact versions and hardware below; they are not current performance claims.
...
- **Full-PDF end-to-end** (`src/savitr/mlx_ocr.py`, all 14 pages of AC01_part001):
...
## Surya MLX backend prototype

The original experiment included a `Backend` subclass that added `method="mlx"`
to `SuryaInferenceManager`. It spawned mlx-vlm's OpenAI server and reused
Surya's `chat_completions_batch`. The prototype worked, but it depended on
Surya internals and was removed before the 0.3.0 release. `MLXSuryaOCR` is the
supported runtime.

## Pipeline integration (#2) — MLX → canonical voter CSV ✅

The prototype that became `src/savitr/rolls/pipeline.py` OCRs a PDF with the MLX
engine, parses voters layout-robustly, and reuses the package's cover metadata
and column schema. Full PDF (AC01_part001):
